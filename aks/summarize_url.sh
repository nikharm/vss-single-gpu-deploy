...
#   ./summarize_url.sh "https://repo.jellyfin.org/archive/jellyfish/media/jellyfish-140-mbps-4k-uhd-h264.mkv"
#
# Prerequisites:
#   - kubectl configured for aks-vss-single-gpu cluster
#   - VSS pods running (kubectl get pods — all should be 1/1 Running)
...
# Output directory — one subdir per run, inside this AKS directory
RUN_DIR="${SCRIPT_DIR}/summaries/${BASENAME}_${TIMESTAMP}"
mkdir -p "$RUN_DIR"

# Log file — captures entire stdout
LOG_FILE="${RUN_DIR}/run.log"
exec > >(tee -a "$LOG_FILE") 2>&1

# --- Ensure kubectl context ---
CONTEXT=$(kubectl config current-context 2>/dev/null || true)
if [[ "$CONTEXT" != *"aks-vss-single-gpu"* ]]; then
  echo "  Switching kubectl context to aks-vss-single-gpu..."
  az aks get-credentials \
    --resource-group rg-nikharm-vss-k8s \
    --name aks-vss-single-gpu \
